...
private func save(email: String, password: String) {
    ...
    private func fetchPerson(email: String, password: String) -> NSManagedObject? {
        ...
        do {
            let fetchedPeople = try managedContext.fetch(fetchRequest)
            return fetchedPeople.first
        } catch {
            print("Failed to fetch records: \(error)")
            return nil
        }
    ...
    @IBAction func enterButtonPressed(_ sender: Any) {
        ...
        if let person = fetchPerson(email: email, password: password) {
            print("Login successful for: \(person)")
            navigateToMainViewController()
        } else {
            showAlert(message: "Email ou senha inválidos")
        }
    }
