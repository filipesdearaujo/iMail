...
class EmaiRandomGenerator {
    static let shared = EmaiRandomGenerator()
    ...
    func generateRandomDate() -> Date {
        ...
        let startDate = dateFormatter.date(from: "01/01/2024 00:00")!
        
        // Intervalo aleatório de até um ano, incluindo horas, minutos e segundos
        let interval = TimeInterval.random(in: 0...365*24*60*60)
        
        return startDate.addingTimeInterval(interval)
    }
    func fetchEmail() -> String? {
        // Obtenha o contexto de CoreData
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return nil }
        let context = appDelegate.persistentContainer.viewContext
        
        // Crie uma requisição para a entidade "Person"
        ...
        do {
            ...
            // Verifique se há resultados e retorne o valor do email
            if let person = result.first as? NSManagedObject, let email = person.value(forKey: "email") as? String {
                return email
            }
        } catch {
    ...
    func getRandomMessage() -> (subject: String, message: String) {
            ...
            let choosed = mensagens.randomElement()!
            let chave = choosed.key
            let value = choosed.value
            return (chave, value)
        }
}
